-- =====================================================
-- WorkSync Phase 3 - Complete MySQL Setup
...
-- =====================================================
-- 7. ENHANCE EXISTING TABLES (SAFE COLUMN ADDITIONS)
-- =====================================================

-- Add Phase 3 columns to existing projects table (MySQL safe way)
-- Check and add project_type column
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = DATABASE() 
     AND TABLE_NAME = 'projects' 
     AND COLUMN_NAME = 'project_type') = 0,
    'ALTER TABLE projects ADD COLUMN project_type ENUM(''KANBAN'', ''TRADITIONAL'', ''HYBRID'') DEFAULT ''TRADITIONAL'' AFTER description',
    'SELECT ''project_type column already exists'' as message'
));
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = DATABASE() 
     AND TABLE_NAME = 'projects' 
     AND COLUMN_NAME = 'estimated_hours') = 0,
    'ALTER TABLE projects ADD COLUMN estimated_hours DECIMAL(8,2) DEFAULT NULL',
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = DATABASE() 
     AND TABLE_NAME = 'projects' 
     AND COLUMN_NAME = 'actual_hours') = 0,
    'ALTER TABLE projects ADD COLUMN actual_hours DECIMAL(8,2) DEFAULT 0.00',
    'SELECT ''actual_hours column already exists'' as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    ...
    'SELECT ''settings column already exists'' as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     ...
     AND COLUMN_NAME = 'story_points') = 0,
    'ALTER TABLE tasks ADD COLUMN story_points INT DEFAULT NULL',
    'SELECT ''story_points column already exists'' as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     ...
     AND TABLE_NAME = 'tasks' 
     AND COLUMN_NAME = 'estimated_hours') = 0,
    'ALTER TABLE tasks ADD COLUMN estimated_hours DECIMAL(6,2) DEFAULT NULL',
    'SELECT ''estimated_hours column already exists'' as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     ...
     AND TABLE_NAME = 'tasks' 
     AND COLUMN_NAME = 'actual_hours') = 0,
    'ALTER TABLE tasks ADD COLUMN actual_hours DECIMAL(6,2) DEFAULT 0.00',
    'SELECT ''actual_hours column already exists'' as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = DATABASE() 
     AND TABLE_NAME = 'tasks' 
     AND COLUMN_NAME = 'labels') = 0,
    'ALTER TABLE tasks ADD COLUMN labels JSON',
    'SELECT ''labels column already exists'' as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Add custom_fields column to tasks
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = DATABASE() 
     AND TABLE_NAME = 'tasks' 
     AND COLUMN_NAME = 'custom_fields') = 0,
    'ALTER TABLE tasks ADD COLUMN custom_fields JSON',
    'SELECT ''custom_fields column already exists'' as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
-- =====================================================
-- 8. PERFORMANCE INDEXES (SAFE CREATION)
-- =====================================================

-- Add performance indexes safely
-- Index for projects project_type
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS 
     WHERE TABLE_SCHEMA = DATABASE() 
     AND TABLE_NAME = 'projects' 
     AND INDEX_NAME = 'idx_projects_type') = 0,
    'CREATE INDEX idx_projects_type ON projects(project_type)',
    'SELECT ''idx_projects_type index already exists'' as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Index for projects template_id
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS 
     WHERE TABLE_SCHEMA = DATABASE() 
     AND TABLE_NAME = 'projects' 
     AND INDEX_NAME = 'idx_projects_template') = 0,
    'CREATE INDEX idx_projects_template ON projects(template_id)',
    'SELECT ''idx_projects_template index already exists'' as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Index for tasks story_points
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS 
     WHERE TABLE_SCHEMA = DATABASE() 
     AND TABLE_NAME = 'tasks' 
     AND INDEX_NAME = 'idx_tasks_story_points') = 0,
    'CREATE INDEX idx_tasks_story_points ON tasks(story_points)',
    'SELECT ''idx_tasks_story_points index already exists'' as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Index for tasks estimated_hours
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS 
     WHERE TABLE_SCHEMA = DATABASE() 
     AND TABLE_NAME = 'tasks' 
     AND INDEX_NAME = 'idx_tasks_estimated_hours') = 0,
    'CREATE INDEX idx_tasks_estimated_hours ON tasks(estimated_hours)',
    'SELECT ''idx_tasks_estimated_hours index already exists'' as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
-- Insert basic Kanban project template
...
-- Create sample Kanban board for the first project (if it exists)
INSERT IGNORE INTO kanban_boards (project_id, name, description, color, position, is_default, created_by)
SELECT 1, 'Main Board', 'Primary Kanban board for project management', '#3B82F6', 0, TRUE, 1
WHERE EXISTS (SELECT 1 FROM projects WHERE id = 1);

-- Get the inserted board ID and create default columns
SET @board_id = (SELECT id FROM kanban_boards WHERE project_id = 1 AND name = 'Main Board' LIMIT 1);

-- Create default columns for the sample board
INSERT IGNORE INTO kanban_columns (board_id, name, color, position, is_done_column) 
SELECT @board_id, 'Backlog', '#6B7280', 0, FALSE WHERE @board_id IS NOT NULL
UNION ALL
SELECT @board_id, 'To Do', '#3B82F6', 1, FALSE WHERE @board_id IS NOT NULL
UNION ALL
SELECT @board_id, 'In Progress', '#F59E0B', 2, FALSE WHERE @board_id IS NOT NULL
UNION ALL
SELECT @board_id, 'Review', '#8B5CF6', 3, FALSE WHERE @board_id IS NOT NULL
UNION ALL
SELECT @board_id, 'Done', '#10B981', 4, TRUE WHERE @board_id IS NOT NULL;
...
SELECT 
    TABLE_NAME as 'Phase 3 Tables',
    TABLE_ROWS as 'Rows'
FROM INFORMATION_SCHEMA.TABLES 
WHERE TABLE_SCHEMA = DATABASE() 
AND (TABLE_NAME LIKE 'kanban_%' 
     OR TABLE_NAME LIKE 'time_tracking_%' 
     OR TABLE_NAME IN ('milestones', 'milestone_deliverables', 'resource_capacity', 'resource_allocations', 'project_templates', 'template_usage', 'workspace_settings', 'user_presence'))
ORDER BY TABLE_NAME;

-- Show sample data counts
SELECT 'Sample Data Summary' as info;
SELECT 
    'Kanban Boards' as table_name, COUNT(*) as count FROM kanban_boards
UNION ALL
SELECT 'Kanban Columns', COUNT(*) FROM kanban_columns
UNION ALL
SELECT 'Time Tracking Settings', COUNT(*) FROM time_tracking_settings
UNION ALL
SELECT 'Project Templates', COUNT(*) FROM project_templates
UNION ALL
...
SELECT '🎉 WorkSync Phase 3 MySQL Setup Complete!' as message,
       'Your backend now includes:' as features_1,
       '📋 Kanban Boards with drag-and-drop' as features_2,
       '⏱️ Professional Time Tracking' as features_3,
       '🎯 Milestone Management' as features_4,
       '👥 Resource Allocation' as features_5,
       '📝 Project Templates' as features_6,
       '📊 Advanced Analytics' as features_7;
